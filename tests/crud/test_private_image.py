...
@parametrize_with_cases("item", cases=CaseImage, has_tag="update")
def test_patch(item: ImageUpdate, private_image_model: PrivateImage) -> None:
    """Update only a subset of the image attributes."""
    db_obj = private_image_mng.patch(obj_in=item, db_obj=private_image_model)
    assert db_obj is not None
    assert isinstance(db_obj, PrivateImage)
...
def test_patch_no_changes(
    item: ImageUpdate, private_image_model: PrivateImage
) -> None:
