...
def test_patch(item: ImageUpdate, shared_image_model: SharedImage) -> None:
    """Update only a subset of the image attributes."""
    db_obj = shared_image_mng.patch(obj_in=item, db_obj=shared_image_model)
    assert db_obj is not None
    assert isinstance(db_obj, SharedImage)
...
def test_patch_no_changes(
    item: ImageUpdate, shared_image_model: SharedImage
) -> None:
