...
def valid_block_storage_quota_id(quota_uid: str) -> BlockStorageQuota:
    ...
    """
    item = block_storage_quota_mgr.get(uid=quota_uid.replace("-", ""))
    if not item:
        raise HTTPException(
...
def valid_compute_quota_id(quota_uid: str) -> ComputeQuota:
    ...
    ------
        NotFoundError: DB entity with given uid not found.
    """
    item = compute_quota_mgr.get(uid=quota_uid.replace("-", ""))
    if not item:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
def valid_network_quota_id(quota_uid: str) -> NetworkQuota:
    ...
    ------
        NotFoundError: DB entity with given uid not found.
    """
    item = network_quota_mgr.get(uid=quota_uid.replace("-", ""))
    if not item:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
def valid_object_store_quota_id(quota_uid: str) -> ObjectStoreQuota:
    ...
    ------
        NotFoundError: DB entity with given uid not found.
    """
    item = object_store_quota_mgr.get(uid=quota_uid.replace("-", ""))
    if not item:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
